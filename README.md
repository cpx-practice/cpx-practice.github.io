...
| `docs/analytics.js` | 분석 탭 — 영역별 성취율, 케이스별 점수(검색·범위·정렬, 펼치면 회차와 놓친 항목) |
| `docs/evaluation.js` | 채점 결과 원문에서 항목별 O/△/X 를 읽어 케이스별로 놓친 항목을 집계 |
...
케이스를 펼치면 왼쪽에 회차별 점수, 오른쪽에 그 케이스에서 자주 놓친 항목이 나옵니다.
전체 집계가 아니라 케이스별입니다 — "가슴통증에서 무엇을 반복해 놓치는가" 가 알고 싶은 것이지,
전 과목 평균이 아니기 때문입니다.

플러그인이 올린 채점 결과는 마크다운 표입니다. `evaluation.js` 가 그 표에서 항목명과
O/△/X 를 뽑아 "몇 번 나왔고 몇 번 놓쳤는지" 를 셉니다 (△는 0.5회, 한 기록 안 중복은 1회).
문서에 별도 필드를 만들지 않고 이미 저장된 원문을 다시 읽는 방식이라, 지난 기록에도 소급 적용됩니다.
