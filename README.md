...
| 필드 | 용도 |
|---|---|
| `uid`, `nickname`, `email` | 누가 보냈는지. 관리자가 목록만 보고 알 수 있게 함께 남깁니다 |
| `text` | 제보 내용 (2000자 이내) |
| `page`, `agent` | 보고 있던 탭과 브라우저. 재현용이고 폼 아래에 무엇을 보내는지 적어뒀습니다 |
| `status` | `open` 또는 `done`. 관리자만 바꿉니다 |

제보에는 이미지 한 장을 붙일 수 있습니다 (파일 선택 또는 본문 칸에 Ctrl+V).
이미지는 `reportImages/{reportId}` 에 따로 들어가고, 목록에는 안 오며 "이미지 보기" 를 누를 때만 가져옵니다.

Cloud Storage 는 쓰지 않습니다. 이 프로젝트에는 버킷이 만들어져 있지 않고(설정에 이름만 있습니다),
새로 켜면 요금제 문제가 걸립니다. 대신 브라우저에서 긴 변 1600px·JPEG 로 줄인 뒤
1500자씩 조각내어 Firestore 에 담습니다 — 기록 전사와 같은 방식입니다.
5MB 짜리 PNG 스크린샷이 100KB 안팎으로 줄어듭니다. 한도를 못 맞추면 저장하지 않고 그렇게 말합니다.

유저는 자기가 낸 제보만 보고, 관리자는 전부 봅니다. 승인 대기 중이어도 제보는 됩니다 —
못 쓰는 사람이 제일 할 말이 많기 때문입니다. 상태 변경과 삭제는 관리자만 할 수 있습니다.
...
| 파일 | 하는 일 |
|---|---|
| `docs/index.html` | 화면 뼈대. 대시보드는 내 기록 / 분석 / Claude / 오류 제보 / 이용 방법 / 설정 (+관리자) 탭 |
...
| `docs/image.js` | 제보에 붙일 이미지를 브라우저에서 줄이고 조각내기 |
