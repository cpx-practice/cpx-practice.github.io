...
| `docs/analytics.js` | 분석 탭 — 영역별 성취율, 케이스별 점수, 케이스 커버리지 |
...
분석 탭은 필터를 타지 않고 항상 전체 기록으로 그립니다. 커버리지에서 해본 케이스를 누르면
케이스별 점수의 해당 줄이 펼쳐지고, 회차마다 "면담 보기" 로 채점 결과와 문진 전사를 볼 수 있습니다.
이때 뜨는 모달은 기록 탭이 쓰는 것과 같은 것이라, `app.js` 가 `renderAnalytics(rows, { onOpenRecord })`
로 여는 함수를 넘겨줍니다. 그래서 `analytics.js` 는 모달 마크업을 몰라도 됩니다.
